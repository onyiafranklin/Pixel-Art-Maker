...
const table=$('#pixelCanvas');//
 //this finction forms a grid
function makeGrid(gridWidthValue,gridHeightValue) {
	
		 const size={
		 gridWidthValue:$('#inputWeight').val(),//width of grid
		 gridHeightValue:$('#inputHeight').val()//height of gro=id
		}
		 	 
				table.find('tr').remove();//remove any previous table that was there before  a new one will be formed
